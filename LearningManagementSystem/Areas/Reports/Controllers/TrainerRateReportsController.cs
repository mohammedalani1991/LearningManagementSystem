...
{
    [Area("Reports")]
    public class TrainerRateReportsController : Controller
    {
        private readonly ISettingService _settingService;
        private readonly ILogService _logService;
        private readonly IStringLocalizer<TrainerRateReportsController> _localizer;
        private readonly ICookieService _cookieService;
        private readonly ITrainerReportService _trainerReportService;

        public TrainerRateReportsController(ICookieService cookieService, ITrainerReportService trainerReportService, ISettingService settingService
            , ILogService logService, IStringLocalizer<TrainerRateReportsController> localizer)
        {
        ...
        [CustomAuthentication(PageName = "TrainerRateReports", PermissionKey = "View")]
        ...
        [CustomAuthentication(PageName = "TrainerRateReports", PermissionKey = "View")]
        [AuditLogFilter(ActionDescription = "List Trainer Rate Reports")]
        public async Task<IActionResult> GetData(int? page, string searchText, int pagination, string table, FilterViewModel filter)
        {
            ...
            var val = _cookieService.GetCookie(Constants.Pagenation.TrainerRateReportsPagination);
            ...
            if (val == null && pagination == 0)
                ...
                pagination = Int32.Parse(_cookieService.CreateCookie(Constants.Pagenation.TrainerRateReportsPagination, pagination.ToString(), 7));
            else
            ...
            var result = _trainerReportService.GetTrainerRateReports(searchText, page, languageId, pagination, filter);
        ...
        public ActionResult GetItems(int? page, int id, string user)
        {
            try
            {
                var result = _trainerReportService.GetManagementItems(page ?? 1, id, user);
        ...
        [HttpGet]
        [AuditLogFilter(ActionDescription = "DownLoad TrainerRate Report")]
        public ActionResult DownloadReport(FilterViewModel filter, string searchText)
        {
            try
            {
                ...
                {
                    DataTable dt = _trainerReportService.DownloadTrainerRateReports(filter, _localizer).Tables[0];
                    wb.Worksheets.Add(dt);
                    using (MemoryStream stream = new MemoryStream())
                    {
                        wb.SaveAs(stream);
                        return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", _localizer["TrainerRateReports"] + "_" + DateTime.Now.ToShortDateString() + ".xlsx");
                    }
                }
            }
            catch (Exception ex)
