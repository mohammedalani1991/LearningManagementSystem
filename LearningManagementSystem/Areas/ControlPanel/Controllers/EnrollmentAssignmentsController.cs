...
{
    [Area("ControlPanel")]
    public class EnrollmentAssignmentsController : Controller
    {
        ...
        public EnrollmentAssignmentsController(
        ...
        [CustomAuthentication(PageName = "EnrollCourseAssignments", PermissionKey = "Create")]
        [AuditLogFilter(ActionDescription = "EnrollCourseAssignment Create Post")]
        public async Task<IActionResult> CreateGroup(EnrollCourseAssignmentViewModel assignmentViewModel)
        {
            try
            {
                if (assignmentViewModel.PublishDate >= assignmentViewModel.PublishEndDate)
                ...
                assignmentViewModel.CreatedBy = User.Identity?.Name;
                var course = _enrollTeacherCourseService.GetEnrollTeacherCoursesIds(assignmentViewModel.CourseId, assignmentViewModel.TeacherId, assignmentViewModel.SemesterId);
                ...
                using (IDbContextTransaction transaction = context.Database.BeginTransaction())
                    try
                    {
                        ...
                        {
                            assignmentViewModel.EnrollTeacherCourseId = item.Id;
                            if (item.LearningMethodId != (int)GeneralEnums.LearningMethodEnum.ElectronicOnce)
                                _assignmentService.AddEnrollCourseAssignment_WithoutUsing(assignmentViewModel, context);
                        }
                        transaction.Commit();
                        return Content("Success");
                    }
