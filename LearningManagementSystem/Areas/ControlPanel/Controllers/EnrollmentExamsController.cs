...
{
    [Area("ControlPanel")]
    public class EnrollmentExamsController : Controller
    {
        ...
        public EnrollmentExamsController(
        ...
        public async Task<IActionResult> GetData(int? page, int pagination, int? CourseId)
        {
            if (page == 0)
                page = 1;
            ...
            var result = _enrollCourseExamService.GetEnrollCourseExams(page, languageId, pagination, CourseId, 0);
        ...
        [CustomAuthentication(PageName = "EnrollCourseExam", PermissionKey = "Create")]
        [AuditLogFilter(ActionDescription = "Enroll Course Exam Create Get")]
        public IActionResult ShowCreateGroup(int? semester, int courseId, int? teacherId)
        {
            var requestCulture = HttpContext.Features.Get<IRequestCultureFeature>();
            var languageId = CultureHelper.GetCurrentLanguageId(requestCulture);
            ...
            var CategoryId = _courseService.GetCourseById(courseId)?.CategoryId;

            ViewBag.ListExamTemplates = _examTemplateService.GetAllExamTemplatet(languageId, courseId, CategoryId);
            ViewBag.TimeZone = TimeZoneInfo.FindSystemTimeZoneById(_settingService.GetOrCreate(Constants.SystemSettings.TimeZone, "UTC").Value).DisplayName;
        ...
        [CustomAuthentication(PageName = "EnrollCourseExam", PermissionKey = "Create")]
        [AuditLogFilter(ActionDescription = "Enroll Course Exam Create Post")]
        public async Task<IActionResult> CreateGroup(EnrollCourseExamViewModel EnrollCourseExamViewModel)
        {
            try
            {
                if (EnrollCourseExamViewModel.IsOnlineLearningMethod == false && EnrollCourseExamViewModel.TestTypeID == (int)GeneralEnums.TestType.Exam)
                ...
                if (EnrollCourseExamViewModel.LanguageId == 0)
                    EnrollCourseExamViewModel.LanguageId = CultureHelper.GetDefaultLanguageId();
                EnrollCourseExamViewModel.CreatedBy = User.Identity?.Name;
                EnrollCourseExamViewModel.TestTypeID = (int)GeneralEnums.TestType.Exam;

                var course = _enrollTeacherCourseService.GetEnrollTeacherCoursesIds(EnrollCourseExamViewModel.CourseId, EnrollCourseExamViewModel.TeacherId, EnrollCourseExamViewModel.SemesterId);
                ...
                using (IDbContextTransaction transaction = context.Database.BeginTransaction())
                    try
                    {
                        ...
                        {
                            EnrollCourseExamViewModel.EnrollTeacherCourseId = item.Id;
                            EnrollCourseExamViewModel.IsOnlineLearningMethod = item.LearningMethodId == (int)GeneralEnums.LearningMethodEnum.ElectronicOnce;
                            var AddedEnrollCourseExam = _enrollCourseExamService.AddEnrollCourseExam_WithoutUsing(EnrollCourseExamViewModel, context);
                            if (EnrollCourseExamViewModel.ExamTemplateId != null && EnrollCourseExamViewModel.ExamTemplateId > 0 && EnrollCourseExamViewModel.TestTypeID == (int)GeneralEnums.TestType.Exam)
                                foreach (var ExamQuestions in _examQuestionService.GetQuestionByTemplateId_WithoutUsing(EnrollCourseExamViewModel.ExamTemplateId.Value, context))
                                {
                                    var enrollCourseExamQuestionViewModel = new EnrollCourseExamQuestionViewModel()
                    ...
                    {
                        transaction.Rollback();
                        _logService.LogException(User.Identity?.Name ?? string.Empty, ex, "Error while Adding Group Exams");
                        return Content("Fail");
                    }
                    ...
                    using (var context = new LearningManagementSystemContext())
                    {
                        using (IDbContextTransaction transaction = context.Database.BeginTransaction())
                        {
                            try
                            {
                                ...
                                var s = _enrollCourseExamQuestionService.DeleteEnrollCourseExamQuestionByEnrollCourseExamID_WithoutUsing(enrollCourseExam.Id, context);
                                if (s)
