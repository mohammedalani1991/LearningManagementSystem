...
namespace LearningManagementSystem.Areas.Trainer.Controllers
{
    ...
    public class EnrollCoursesContentController : Controller
    {
        ...
        public async Task<IActionResult> GetData(int? page, int pagination, string table, int? CourseId)
        {
            ...
            var result = _enrollSectionOfCourseService.GetEnrollSectionOfCourses(TeacherId, 0, page, languageId, pagination, CourseId);
        ...
        public IActionResult ShowCreate(int EnrollTeacherCourseId, int languageId = 0)
        {
            var requestCulture = HttpContext.Features.Get<IRequestCultureFeature>();
            ...
            var TrainerDetails = _trainerService.GetTrainerByContactId(_userProfileService.GetUserProfileByUsername(User.Identity.Name).Contact.Id);
            //ViewBag.TrainerCourses = _enrollTeacherCourseService.GetEnrollTeacherCourseByTeacherId(TrainerDetails.Id, languageId);
        ...
        [CustomAuthentication(PageName = "EnrollCoursesContent", PermissionKey = "FetchCoursesContent")]
        [AuditLogFilter(ActionDescription = "FetchCoursesContent")]
        public async Task<IActionResult> FetchCoursesContent(int id)
        {
            try
            {
                _enrollSectionOfCourseService.FetchCoursesContent(id);
                return Ok();
            }
            catch (Exception ex)
            {
        ...
        [CustomAuthentication(PageName = "EnrollCoursesContent", PermissionKey = "FetchCoursesContent")]
        public async Task<IActionResult> RegenerateCourseData(int id)
        {
                ...
                using (var context = new LearningManagementSystemContext())
                {
                    using (IDbContextTransaction transaction = context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            _enrollSectionOfCourseService.DeleteEnrollSectionByEnrollTeacherCourseID_WithoutUsing(CurrentEnrollTeacherCourse.Id, context);
                            ...
                            foreach (var Section in _sectionOfCourseService.GetSectionOfCourseByCourseId_WithoutUsing(CurrentEnrollTeacherCourse.CourseId, context))
                            {
                                ...
                                var AddedEnrollSectionOfCourse = _enrollSectionOfCourseService.AddEnrollSectionOfCourseForAdmin(Section, CurrentEnrollTeacherCourse.Id, context);
